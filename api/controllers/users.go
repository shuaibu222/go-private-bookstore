...
func UpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	if founded.ID == uint(intId) {
		...
		if userUpdate.UploadedProfileImage != "" {
			founded.UploadedProfileImage = userUpdate.UploadedProfileImage
			path := founded.UploadedProfileImage
			url := utils.UploadUserImage(path)

			// updating this anonymous function with the new profile image url
			founded.AvatarURL = url
		}
...
// TODO: configure removal well

func DeleteProfileImageFromS3(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)
	...
	user, db := models.GetUserById(intId)
	...
	if user.AvatarURL != "" {
		...
		user.AvatarURL = "" // Clear the AvatarURL in the database
